...
describe("Custom Tests", function () {
  ...
  it("[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances", async () => {
    ...
    const alice = (await ethers.getSigners())[4]; //generate a valid ETH address
    const aliceKeypair = new Keypair(); // Alice's private and public keys
    ...
    const recipientBalance = await token.balanceOf(recipient); // 0.08
    expect(recipientBalance).to.be.equal(aliceWithdrawAmount);
    const omniBridgeBalance = await token.balanceOf(omniBridge.address); //0
    expect(omniBridgeBalance).to.be.equal(0);
    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address); //0.02
  ...
  it("[assignment] iii. see assignment doc for details", async () => {
    // [assignment] complete code here
    const { tornadoPool, token, omniBridge } = await loadFixture(fixture);
    const alice = (await ethers.getSigners())[4]; // alice's withdrawal address
    const bob = (await ethers.getSigners())[5]; //bob's withdrawal address
    ...
    const bobEthAddress = bob.address;
    ...
    //alice withdraws all of her funds from the shielded pool
    ...
    const bobBalance = await token.balanceOf(bobEthAddress); //0.06
    expect(bobBalance).to.be.equal(bobWithdrawAmount);
    const aliceBalance = await token.balanceOf(aliceAddress); //0
    expect(aliceBalance).to.be.equal(0);
    const omniBridgeBalance = await token.balanceOf(omniBridge.address); //0.07
    expect(omniBridgeBalance).to.be.equal(aliceWithdrawAmount);
    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address); //0
